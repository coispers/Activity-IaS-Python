...
- part1/part1_recon_vulnerability_report.md
- part2/part2_encryption_report.md
- part3/part3_rbac_report.md
- part4/part4_zero_trust_report.md
- part5/part5_test_report.md
- report.md (consolidated summary; ignored by git)

## Common Troubleshooting

1. Error: Missing access token

Cause:

- You are calling file endpoint directly without a token in hardened mode.

Fix:

- Login first through auth server and use the returned accessToken in requests/tests.

2. Error: Invalid token signature

Cause:

- INTER_SERVICE_SECRET differs between auth_server.py and file_server.py runtime.

Fix:

- Restart both servers with the same INTER_SERVICE_SECRET value.

3. Port already in use / startup failure

Fix:

- Stop old python server processes and restart both services.

## Security Notes

- Replay protection is implemented by token jti one-time use tracking in file service.
- Current replay tracking is in-memory; for production, use shared persistent storage (for example Redis).
